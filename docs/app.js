const definitions = [
  ...
  ['per_capita_nni_inr', 'Per capita Net National Income', 'Per capita NNI at current prices in Indian Rupees', 'Annual', '#3fb950', ' INR', 'https://www.mospi.gov.in/', 'Per capita Net National Income (NNI) measures national income divided by population in current Indian Rupees. It expanded from ₹63,462 in FY2012 to ₹2.18 Lakh in FY2026.\n\nSourced from MOSPI National Accounts Statistics and official Economic Survey statements.'],
  ['per_capita_income_ppp', 'Per capita GNI in PPP dollars', 'GNI per capita in Purchasing Power Parity current international $', 'Annual', '#58a6ff', ' USD PPP', 'https://data.worldbank.org/indicator/NY.GNP.PCAP.PP.CD', 'GNI per capita converted to international dollars using purchasing power parity (PPP) rates, accounting for domestic price differences.\n\nSourced from World Bank Open Data indicator NY.GNP.PCAP.PP.CD.'],
  ['infant_mortality_rate', 'Infant Mortality Rate (IMR)', 'Infant deaths per 1,000 live births', 'Annual', '#ff7b72', ' per 1k births', 'https://data.worldbank.org/indicator/SP.DYN.IMRT.IN', 'Infant Mortality Rate (IMR) measures deaths of infants under one year per 1,000 live births. India achieved a steep reduction from 88.6 in 1990 down to 23.0.\n\nSourced from Sample Registration System (SRS) Bulletins & World Bank.'],
  ['maternal_mortality_ratio', 'Maternal Mortality Ratio (MMR)', 'Maternal deaths per 100,000 live births', 'Annual', '#f778ba', ' per 100k births', 'https://www.censusindia.gov.in/', 'Maternal Mortality Ratio (MMR) measures maternal deaths per 100,000 live births. India dropped below the SDG target milestone down to 89 per 100k.\n\nSourced from SRS Special Bulletins on Maternal Mortality & WHO/UNICEF.'],
  ['tb_disease_incidence', 'Tuberculosis (TB) incidence rate', 'TB incidence rate per 100,000 population', 'Annual', '#ffa657', ' per 100k', 'https://nikshay.in/', 'Annual Tuberculosis (TB) incidence rate per 100,000 population tracking nationwide active case finding and treatment under National TB Elimination Programme.\n\nSourced from Central TB Division Nikshay Portal & WHO.'],
  ['non_communicable_diseases_burden', 'Disease burden: NCDs vs Communicable', '% Share of total disease DALYs burden', 'Annual / Survey', '#ff7b72', '% DALYs', 'https://www.icmr.gov.in/', 'Epidemiological transition chart comparing Non-Communicable Diseases (NCDs: cardiovascular, diabetes, cancer) vs Communicable/Maternal/Child disease burden.\n\nSourced from ICMR India State-Level Disease Burden Initiative & IHME.'],
  ['health_expenditure_gdp', 'Government health spend vs Out-of-pocket', 'Govt spending % GDP vs Out-of-pocket expenditure %', 'Annual', '#3fb950', '%', 'https://main.mohfw.gov.in/', 'Health financing transition comparing Government Health Expenditure (% of GDP) alongside reduction in Out-of-Pocket Expenditure (% total health spend).\n\nSourced from National Health Accounts (NHA) Estimates & MoHFW.'],
  ['state_per_capita_income', 'Per capita income by state', 'Net State Domestic Product per capita in INR Lakhs', 'Annual / State', '#3fb950', ' INR Lakhs', 'https://www.mospi.gov.in/', 'Per capita Net State Domestic Product across 28 States & Union Territories (Telangana ₹3.12L, Karnataka ₹3.01L, Haryana ₹2.96L, Tamil Nadu ₹2.75L, Gujarat ₹2.73L).\n\nSourced from MOSPI & RBI Handbook of Statistics on Indian States.'],
  ['state_own_tax_revenue', 'State Own Tax Revenue (SOTR)', 'State Own Tax Revenue in INR Lakh Crores', 'Annual / State', '#58a6ff', ' INR Lakh Cr', 'https://www.rbi.org.in/', 'State fiscal autonomy indicator tracking State Own Tax Revenue (SOTR) collections across major state economies.\n\nSourced from RBI State Finances Study & State Budgets.'],
  ['state_gst_collections', 'Monthly State GST collections', 'Average monthly GST revenue by state', 'Monthly / State', '#f6c344', ' INR Cr', 'https://www.gst.gov.in/', 'Monthly State GST (SGST) collections comparing commercial consumption volume across states.\n\nSourced from GSTN Portal & Ministry of Finance.'],
  ['state_manufacturing_gsdp_share', 'Manufacturing share of GSDP', 'Manufacturing sector % share of GSDP', 'Annual / State', '#ff9933', '% GSDP', 'https://www.mospi.gov.in/', 'Manufacturing depth comparing industrial contribution to state GDP (Gujarat 36.5%, Himachal 29.8%, Tamil Nadu 24.2%, Maharashtra 21.5%).\n\nSourced from MOSPI National Accounts & RBI State Statistics.'],
  ['state_pmjdy_bank_accounts', 'PM Jan Dhan accounts by state', 'Cumulative PMJDY bank accounts opened in Crores', 'Annual / State', '#79c0ff', ' Crore accounts', 'https://pmjdy.gov.in/', 'State-wise distribution of PM Jan Dhan Yojana financial inclusion bank accounts (Uttar Pradesh 9.25 Cr, Bihar 5.82 Cr, West Bengal 4.95 Cr).\n\nSourced from Department of Financial Services, Ministry of Finance.'],
  ['state_pmjay_health_cards', 'Ayushman Bharat cards by state', 'Ayushman Bharat PM-JAY health cards issued in Crores', 'Annual / State', '#f778ba', ' Crore cards', 'https://pmjay.gov.in/', 'State-wise distribution of Ayushman Bharat health protection cards issued to vulnerable households.\n\nSourced from National Health Authority (NHA) PM-JAY Open Dashboard.'],
  ['state_pmsvanidhi_street_vendors', 'PM SVANidhi vendor credit by state', 'Micro-credit working capital loans disbursed in Lakh vendors', 'Annual / State', '#3fb950', ' Lakh vendors', 'https://pmsvanidhi.mohua.gov.in/', 'State-level street vendor micro-credit loan disbursements under PM SVANidhi scheme.\n\nSourced from Ministry of Housing and Urban Affairs (MoHUA).'],
  ['state_renewable_energy_capacity', 'Renewable energy capacity by state', 'Installed Solar & Wind power capacity in GW', 'Annual / State', '#f6c344', ' GW', 'https://mnre.gov.in/', 'Clean energy generation capacity comparing installed renewable power across states (Rajasthan 28.5 GW, Gujarat 26.2 GW, Tamil Nadu 21.4 GW).\n\nSourced from Ministry of New and Renewable Energy (MNRE) & CEA.'],
  ['state_national_highway_length', 'National Highway length by state', 'National Highways network length in Kilometres', 'Annual / State', '#56d364', ' km', 'https://morth.nic.in/', 'National Highway network density comparing highway infrastructure length across states (Maharashtra 18,459 km, UP 12,272 km, Rajasthan 10,618 km).\n\nSourced from Ministry of Road Transport and Highways (MoRTH).'],
  ['state_literacy_rates', 'Literacy rates across States & UTs', 'Literacy rate % across 28 States and 8 UTs', 'Census / PLFS', '#bc8cff', '%', 'https://www.mospi.gov.in/', 'State-wise literacy rate comparison across India\'s federal landscape (Kerala 96.2%, Mizoram 91.3%, Goa 88.7%, Himachal 83.8%, Maharashtra 82.3%).\n\nSourced from MOSPI Periodic Labour Force Survey & Census Estimates.'],
  ['state_infant_mortality_rate', 'Infant Mortality Rate by state', 'Infant deaths per 1,000 live births across states', 'Annual / State', '#ff7b72', ' per 1k births', 'https://www.censusindia.gov.in/', 'State-level health outcome comparing Infant Mortality Rate per 1k live births (Kerala 6, Delhi 12, Tamil Nadu 13, Maharashtra 16 vs UP 38, MP 43).\n\nSourced from Sample Registration System (SRS) Registrar General of India.'],
  ['state_life_expectancy', 'Life expectancy at birth by state', 'Average life expectancy at birth in years across states', 'Annual / State', '#79c0ff', ' years', 'https://www.censusindia.gov.in/', 'State-level longevity comparing average life expectancy at birth (Kerala 75.2 yrs, Delhi 74.8 yrs, J&K 74.2 yrs, Himachal 73.5 yrs, Punjab 72.8 yrs).\n\nSourced from SRS Abridged Life Tables, RGI.'],
  ['state_ev_adoption_charging', 'Electric Vehicle adoption by state', 'EV registrations & POS charging stations in Thousands', 'Annual / State', '#3fb950', ' Thousand EVs', 'https://vahan.parivahan.gov.in/', 'State-wise electric mobility transition comparing EV vehicle registrations and charging station infrastructure.\n\nSourced from Ministry of Heavy Industries VAHAN Portal.'],
  ['state_export_preparedness_index', 'Export Preparedness Index by state', 'NITI Aayog EPI overall composite score out of 100', 'Annual / State', '#58a6ff', ' Index', 'https://www.niti.gov.in/', 'State export competitiveness benchmark evaluating policy ecosystem, business environment, and export performance (Tamil Nadu 79.2, Maharashtra 78.6, Karnataka 76.4).\n\nSourced from NITI Aayog Export Preparedness Index.'],
  ['state_ease_of_doing_business', 'Ease of Doing Business by state', 'DPIIT BRAP Top Achievers Business Reform score %', 'Annual / State', '#a371f7', '% score', 'https://dpiit.gov.in/', 'State business environment ranking evaluating Business Reform Action Plan (BRAP) implementation.\n\nSourced from DPIIT Ease of Doing Business State Rankings.'],
  ['northeast_insurgency_fatalities', 'North East insurgency fatalities', 'Annual fatalities in NE insurgent violence', 'Annual (2000-2025)', '#ff7b72', ' deaths', 'https://www.mha.gov.in/', 'Decadal fatality trajectory in insurgent violence across North Eastern states showing an 85%+ structural drop in security casualties.\n\nSourced from MHA North East Division & SATP North East Insurgency Database.'],
  ['northeast_insurgency_incidents', 'North East insurgent incidents', 'Annual registered insurgent incidents in North East', 'Annual (2014-2025)', '#ffa657', ' incidents', 'https://www.mha.gov.in/', 'Annual registered insurgent incidents across NE region showing pacification following major peace accords.\n\nSourced from Ministry of Home Affairs Annual Reports (North East Division).'],
  ['kashmir_insurgency_fatalities', 'Jammu & Kashmir insurgency deaths', 'Annual fatalities in J&K terrorism & insurgency', 'Annual (2000-2025)', '#f85149', ' deaths', 'https://www.mha.gov.in/', 'Annual total fatalities (civilians, security personnel, and terrorists) in J&K counter-terror operations dropping from 4,500+ in 2001 to under 100.\n\nSourced from MHA J&K Division & SATP Casualty Database.'],
  ['kashmir_terror_incidents', 'Jammu & Kashmir terror incidents', 'Annual registered terrorist incidents in J&K', 'Annual (2014-2025)', '#ff7b72', ' incidents', 'https://www.mha.gov.in/', 'Annual registered terrorist incidents in J&K showing a steep structural decline post-2019.\n\nSourced from Ministry of Home Affairs J&K Division Performance Reports.'],
  ['khalistan_insurgency_historical', 'Punjab militancy historical fatalities', 'Annual deaths during Punjab militancy period', 'Historical (1981-1995)', '#d29922', ' deaths', 'https://www.satp.org/', 'Historical annual fatality archive tracking the Punjab militancy period (1981-1995) prior to full restoration of peace.\n\nSourced from SATP Punjab Militancy Database & MHA Historical Security Reports.'],
...
const eraFor = key => {
  if (['defence_expenditure', 'defence_exports', 'defence_production', 'defence_stockpile', 'defence_production_exports', 'defence_budget_share', 'defence_rd_budget', 'defence_capital_acquisition', 'sipri_arms_imports', 'upi', 'upi_transaction_value', 'credit_card_spends', 'debit_card_spends', 'credit_card_in_circulation', 'debit_card_in_circulation', 'rbi_digital_payments_index', 'national_highways_built', 'railway_electrification', 'renewable_energy_capacity', 'civil_aviation_passengers', 'telecom_broadband_subscribers', 'auto_production_volume', 'crude_steel_production', 'electronics_manufacturing', 'pharma_exports_trajectory', 'textiles_apparel_exports', 'iron_ore_production', 'bauxite_aluminium_output', 'copper_production', 'manganese_chromite_production', 'coal_production', 'lignite_production', 'crude_petroleum_domestic', 'natural_gas_production', 'limestone_production', 'rare_earths_critical_minerals', 'rock_phosphate_gypsum', 'mineral_metal_exports', 'iron_ore_exports', 'refined_petroleum_exports', 'critical_mineral_import_dependency', 'state_gsdp_comparison', 'state_per_capita_income', 'state_fdi_inflows', 'state_debt_to_gsdp', 'state_own_tax_revenue', 'state_gst_collections', 'state_operating_factories', 'state_manufacturing_gsdp_share', 'state_pmay_homes', 'state_jjm_water_coverage', 'state_mudra_loans', 'state_pmjdy_bank_accounts', 'state_pmjay_health_cards', 'state_pmsvanidhi_street_vendors', 'state_renewable_energy_capacity', 'state_national_highway_length', 'state_literacy_rates', 'state_infant_mortality_rate', 'state_life_expectancy', 'state_ev_adoption_charging', 'state_export_preparedness_index', 'state_ease_of_doing_business', 'ndps_drug_cases', 'child_protection_pocso', 'road_accidents_fatalities', 'police_per_population', 'kashmir_stone_pelting_law_order', 'northeast_insurgents_surrendered', 'punjab_border_drone_seizures', 'internet_users', 'cyber_crime', 'sectoral_market_indices'].includes(key)) return 'modern';
  ...
  if (['gdp_per_capita', 'population', 'electricity_access', 'iip', 'power_consumption', 'rail_freight', 'port_cargo', 'broad_money', 'bank_credit', 'tax_revenue', 'ncrb_ipc_crime_rate', 'khalistan_insurgency_historical', 'punjab_militancy_civilian_deaths', 'punjab_militancy_security_deaths', 'punjab_militants_eliminated'].includes(key)) return 'republic';
  if (['rupee', 'cpi', 'wpi', 'homicide_rate', 'terror_attacks', 'terror_fatalities'].includes(key)) return 'freedom';
  if (['lwe_incidents', 'lwe_civilian_casualties', 'lwe_security_force_casualties', 'lwe_perpetrator_casualties', 'northeast_insurgency_fatalities', 'northeast_insurgency_incidents', 'northeast_civilian_casualties', 'northeast_security_forces_killed', 'kashmir_insurgency_fatalities', 'kashmir_terror_incidents', 'kashmir_civilian_casualties', 'kashmir_security_forces_killed', 'kashmir_terrorists_neutralized', 'ncrb_crime', 'life_expectancy'].includes(key)) return 'republic';
